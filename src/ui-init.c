...
void textui_init(void)
{
	...
	if (ANGBAND_TERM_MAX > 2) default_window_flag[3] = (PW_MONLIST);
	if (ANGBAND_TERM_MAX > 3) default_window_flag[4] = (PW_ITEMLIST);
	if (ANGBAND_TERM_MAX > 4) default_window_flag[5] = (PW_MONSTER | PW_OBJECT);
	if (ANGBAND_TERM_MAX > 5) default_window_flag[2] = (PW_INVEN);
